...
class TestCaseGenerator {
    ...
    async checkAPIStatus() {
        ...
        try {
            ...
            console.log('📊 Full API Status Response:', JSON.stringify(data, null, 2));
            
            // Log individual service statuses with detailed info
            if (data.openai) {
                const openaiStatus = data.openai.status === 'active' ? '✅' : '❌';
                console.log(`${openaiStatus} OpenAI API:`, {
                    status: data.openai.status,
                    available: data.openai.available,
                    message: data.openai.message
                });
            }
            
            if (data.local_llama) {
                const llamaStatus = data.local_llama.available ? '✅' : '❌';
                console.log(`${llamaStatus} Local Llama:`, {
                    status: data.local_llama.status,
                    available: data.local_llama.available,
                    message: data.local_llama.message
                });
            }
            
            if (data.enhanced_mock) {
                const mockStatus = data.enhanced_mock.available ? '✅' : '❌';
                console.log(`${mockStatus} Enhanced Mock:`, {
                    status: data.enhanced_mock.status,
                    available: data.enhanced_mock.available,
                    message: data.enhanced_mock.message
                });
            }
            
            if (data.primary_method) {
                console.log(`🎯 Primary Generation Method: ${data.primary_method}`);
            }
            
            // Show summary
            const totalServices = Object.keys(data).filter(key => key !== 'primary_method').length;
            const activeServices = Object.values(data)
                .filter(service => service && service.available)
                .length;
            
            console.log(`📈 Services Summary: ${activeServices}/${totalServices} active`);
            ...
            this.showAlert('Failed to check API status. Check console for details.', 'warning');
            return null;
        } finally {
            console.groupEnd();
        }
...
Scenario: Test ${method} Request
  ...
  * print 'Response:', response`;
    ...
    async runAutomationScript() {
        console.log('🧪 Starting Karate test automation...');
        console.group('Test Execution');
        
        // Check if we have generated test cases to use
        if (!this.testCases || this.testCases.length === 0) {
            console.warn('⚠️ No test cases generated');
            this.showAlert('Please generate test cases first before running automation', 'warning');
            console.groupEnd();
            return;
        }
        ...
        const apiData = {
            ...
            body: document.getElementById('payload').value,
            resourceId: document.getElementById('resourceId').value,
            generatedTestCases: this.testCases // Include generated test cases
        };

        console.log('📋 Test Configuration:', {
            endpoint: apiData.apiEndpoint,
            method: apiData.method,
            hasCredentials: !!(apiData.username && apiData.password),
            hasBody: !!apiData.body,
            hasResourceId: !!apiData.resourceId,
            generatedTestCasesCount: this.testCases.length
        });

        console.log('📊 Generated test cases to execute:', this.testCases.length);
        console.log('📝 Test cases preview:', this.testCases.slice(0, 3).map(tc => tc['Test Scenario'] || tc.scenario || 'Unknown'));

        if (!apiData.apiEndpoint) {
            console.warn('❌ Missing API endpoint');
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('❌ Automation request failed:', data);
