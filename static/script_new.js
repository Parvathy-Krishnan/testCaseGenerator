class KarateTestGenerator {
    constructor() {
        ...
        const stopProcessBtnEl = document.getElementById('stopProcessBtn');
        if (stopProcessBtnEl) {
            stopProcessBtnEl.addEventListener('click', () => this.cancelCurrentTask());
        }
    ...
    toggleAuthFields() {
        ...
        } else if (['bearer', 'oauth'].includes(authValue)) {
    ...
    async generateTestCases(e) {
        ...
        // Set processing state and generate task ID
        ...
        const formData = new FormData();
            ...
            // Add task ID to form data for server tracking
    ...
    async runKarateAutomation() {
        ...
        try {
            ...
            } else if (['bearer', 'oauth'].includes(authType)) {
            ...
            // Add task ID for server tracking
            requestData.task_id = this.currentTaskId;
    ...
    displayResults(results) {
        ...
        results.testResults.forEach((test, index) => {
            ...
            // Use the accurate expected status from the backend, with a fallback
            const expectedCode = test.expectedStatus || this.getExpectedStatusCode(test.scenario);
    ...
    getFormData() {
        ...
        return {
            ...
            token: ['bearer', 'oauth'].includes(authType) ? (document.getElementById('token').value || '') : '',
            body: document.getElementById('payload').value || '',
            resourceId: document.getElementById('resourceId').value || '',
            acceptHeader: document.getElementById('acceptHeader').value || ''
        };
...
Request Body: ${formData?.body || 'None'}`;
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="p-3 bg-info bg-opacity-10 rounded border border-info text-center">
                                        ...
                                        <div class="mt-2">
                                            <span class="badge ${this.getStatusCodeClass(expectedCode)} fs-4">${expectedCode}</span>
                                        </div>
                                        <small class="text-muted">${this.getStatusCodeDescription(expectedCode)}</small>
                                    </div>
                                </div>
                            </div>
                            <div class="text-center mt-2">
                                <span class="badge ${test.statusCode == expectedCode ? 'bg-success' : 'bg-danger'} fs-5">
                                    ${test.statusCode == expectedCode ? '✓ EXACT MATCH' : '✗ CODE MISMATCH'}
                                </span>
                                <div class="mt-1">
                                    <small class="text-muted">
                                        ${test.statusCode == expectedCode ? 
                                          `API returned expected ${expectedCode} response code` : 
                                          `Expected ${expectedCode}, but got ${test.statusCode}`}
                                    </small>
                                </div>
                            </div>
                        </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <div class="mt-2 p-3 bg-light rounded border text-center">
                            ...
                            <div class="mt-1">
                                <small class="text-muted">${test.justification || (test.status === 'PASSED' ? 'Test executed successfully - All assertions passed' : 'Test failed - One or more assertions did not pass')}</small>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    showLoading(message, title = 'Processing...') {
        ...
        const tryShowLoading = () => {
            try {
                ...
                // Show stop button if we're processing
                if (this.isProcessing) {
                    console.log('Processing is true, looking for stop button...');
    ...
    async cancelCurrentTask() {
        console.log('Cancel button clicked, currentTaskId:', this.currentTaskId);
        console.log('isProcessing:', this.isProcessing);
        
        // Add a guard to prevent multiple cancellation requests for the same task
        if (!this.isProcessing) {
            console.warn('Cancellation already in progress or task is not running.');
            return;
        }
        
        if (!this.currentTaskId) {
            this.showAlert('No active task to cancel.', 'warning');
            return;
        }

        console.log('Sending cancel request for task:', this.currentTaskId);
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showAlert('Task cancelled successfully.', 'success');
                ...
                this.showAlert(result.error || 'Failed to cancel task.', 'danger');
            }
        } catch (error) {
            console.error('Error cancelling task:', error);
            this.showAlert('Error cancelling task.', 'danger');
        }
    }
